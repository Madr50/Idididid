...
async def on_startup():
    ...
    # Set bot commands — Aiogram 3.7+ format
    await bot.set_my_commands([
        BotCommand(command="start", description="بدء البوت / Start the bot"),
        BotCommand(command="menu", description="القائمة الرئيسية / Main menu"),
        BotCommand(command="profile", description="حسابي / My profile"),
        BotCommand(command="support", description="الدعم الفني / Support"),
        BotCommand(command="rules", description="القوانين / Rules"),
    ])
...
async def main():
    ...
    # Initialize Bot and Dispatcher — Aiogram 3.7+ format
